...
class ParsingModel(nn.Module):
    def __init__(self, embeddings, out_size, config:parser_utils.Config):
        ...
        if config.use_dep:
            self.hidden_layer = nn.Linear(self.embedding_size * 84, self.hidden_size, bias=True)
        else:
            self.hidden_layer = nn.Linear(self.embedding_size * 60, self.hidden_size, bias=True)
